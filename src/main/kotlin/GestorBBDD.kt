...
class GestorBBDD {
    ...
    fun selectAllPedidosSinTaller(): List<Pedido>? =
        manager.createQuery("from Pedido where taller=null")?.resultList as List<Pedido>
    ...
    fun clientesAsociados(cif: String): Set<Cliente>? {
        val taller = selectTallerByCif(cif)

        return taller!!.clientes
    }
    ...
    fun talleresCliente(dni: String): Set<Taller>? {
        val cliente = selectClienteByDni(dni)
        return cliente.talleres
    }
